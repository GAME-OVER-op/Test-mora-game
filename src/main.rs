...
use crate::{
    ...
    domain::{base_index_from_ratio, Domain},
    fan::Fan,
    ...
    user_config::{load_or_init as load_config_or_init, CONFIG_PATH},
};
...
/// `1` = cooler enabled (runs by temperature/params), `0` = fully disabled.
fn read_fan_status_prop() -> Option<bool> {
    let out = std::process::Command::new("getprop")
        .arg("persist.perf.fan.status")
        .output()
        .ok()?;
...
/// Default the cooler status to ON, but only if the property is not yet set in the system.
fn ensure_fan_status_default() {
    if read_fan_status_prop().is_none() {
        let _ = std::process::Command::new("setprop")
            .args(["persist.perf.fan.status", "1"])
            .status();
    }
}
...
fn main() {
    ...
    // The game registry + trigger coordinates live entirely in persistent
    // system properties (persist.mora.g.<package> = <left>;<right>). No files.
    let games_rt = load_from_props();
    println!(
        "GAMES: list {} pkgs (driver {})",
        games_rt.pkg_set.len(),
    ...
    let mut cfg = load_config_or_init(cfg_path.as_path());
    led_props::ensure_defaults(&cfg);
...
let mut s = shared.write().unwrap();
    ...
    // Balance ceiling = ~50% of each frequency table.
    let cpu0_cap50 = CPU0_FREQS[base_index_from_ratio(CPU0_FREQS, 0.5)];
    let cpu2_cap50 = CPU2_FREQS[base_index_from_ratio(CPU2_FREQS, 0.5)];
    let cpu5_cap50 = CPU5_FREQS[base_index_from_ratio(CPU5_FREQS, 0.5)];
    let cpu7_cap50 = CPU7_FREQS[base_index_from_ratio(CPU7_FREQS, 0.5)];
    let gpu_cap50 = gpu.freqs[base_index_from_ratio(gpu.freqs, 0.5)];

    // Absolute tops (full performance / "За пределами").
    let cpu0_top = *CPU0_FREQS.last().unwrap();
    let cpu2_top = *CPU2_FREQS.last().unwrap();
    let cpu5_top = *CPU5_FREQS.last().unwrap();
    let cpu7_top = *CPU7_FREQS.last().unwrap();
    let gpu_top = *gpu.freqs.last().unwrap();
    ...
    let policy0_gov_path = PathBuf::from(POLICY0_GOV);
    let policy2_gov_path = PathBuf::from(POLICY2_GOV);
    let policy5_gov_path = PathBuf::from(POLICY5_GOV);
    ...
    // 1 = Баланс, 2 = Подъём, 3 = За пределами. Applied only while a game is foreground.
    let mut perf_mode: u8 = 1;
    // Last applied (game_mode, perf_mode) key, so caps/governors are re-applied only on change.
    let mut last_applied_key: (bool, u8) = (false, 0);
    // Fan baseline while a game is foreground (fixed; per-game override removed).
    let game_fan_min_level: u8 = GAME_FAN_BASE;
    // Master cooler switch from persist.perf.fan.status (1 = on, 0 = off). Default ON.
    let mut cooler_enabled: bool = read_fan_status_prop().unwrap_or(true);
    ...
    loop {
        ...
        if screen_on && now.duration_since(last_game_check) >= game_check_every {
            ...
            // ------------------------------------------------------------
            // Apply performance mode. The three modes act ONLY while a game
            // is in the foreground; with no game we keep the normal idle
            // behaviour. Caps/governors are re-applied only when the
            // (game_mode, perf_mode) combination changes.
            // ------------------------------------------------------------
            let cur_key = if game_mode { (true, perf_mode) } else { (false, 0u8) };
            if cur_key != last_applied_key {
                last_applied_key = cur_key;

                // (c0_min,c0_max, c2_min,c2_max, c5_min,c5_max, c7_min,c7_max, g_min,g_max, governor, label)
                let (c0_min, c0_max, c2_min, c2_max, c5_min, c5_max, c7_min, c7_max, g_min, g_max, gov, label):
                    (u64, u64, u64, u64, u64, u64, u64, u64, u64, u64, &str, &str) =
                    if !game_mode {
                        (cpu0_min_normal, cpu0_top, cpu2_min_normal, cpu2_top, cpu5_min_normal, cpu5_top,
                         cpu7_min_normal, cpu7_top, gpu_min_normal, gpu_top, GOV_NORMAL, "normal (no game)")
                    } else {
                        match perf_mode {
                            // Подъём: full range, floor 0%, dynamic governor.
                            2 => (cpu0_min_normal, cpu0_top, cpu2_min_normal, cpu2_top, cpu5_min_normal, cpu5_top,
                                  cpu7_min_normal, cpu7_top, gpu_min_normal, gpu_top, GOV_NORMAL, "boost (Подъём) full range"),
                            // За пределами: hard max, performance governor on all clusters.
                            3 => (cpu0_top, cpu0_top, cpu2_top, cpu2_top, cpu5_top, cpu5_top,
                                  cpu7_top, cpu7_top, gpu_top, gpu_top, GOV_GAME, "beyond (За пределами) hard max"),
                            // Баланс (1 / default): ceiling 50%, floor 0%, dynamic governor.
                            _ => (cpu0_min_normal, cpu0_cap50, cpu2_min_normal, cpu2_cap50, cpu5_min_normal, cpu5_cap50,
                                  cpu7_min_normal, cpu7_cap50, gpu_min_normal, gpu_cap50, GOV_NORMAL, "balance (Баланс) cap 50%"),
                        }
                    };

                cpu0.min_freq = c0_min; cpu0.max_freq = c0_max;
                cpu2.min_freq = c2_min; cpu2.max_freq = c2_max;
                cpu5.min_freq = c5_min; cpu5.max_freq = c5_max;
                cpu7.min_freq = c7_min; cpu7.max_freq = c7_max;
                gpu.min_freq = g_min; gpu.max_freq = g_max;

                let _ = write_str_if_needed(&policy0_gov_path, gov, &mut cache_str, true);
                let _ = write_str_if_needed(&policy2_gov_path, gov, &mut cache_str, true);
                let _ = write_str_if_needed(&policy5_gov_path, gov, &mut cache_str, true);
                let _ = write_str_if_needed(&policy7_gov_path, gov, &mut cache_str, true);

                println!("PERF: {}", label);
            }
        }

        // screen transition (track screen-off start for the 30-min core saver)
...
// fan
        if let Some(f) = fan.as_mut() {
            if cfg.use_phone_cooler && cooler_enabled {
        ...
        // idle mode
        let idle_cond = !screen_on && max_cpu_cluster < IDLE_CPU_MAX && ug < IDLE_GPU_MAX;
        ...
        // "За пределами": hard-pin every domain to its maximum and bypass
        // thermal caps (GPU turbo lives here now). Other modes scale
        // dynamically within their configured window.
        let beyond = game_mode && screen_on && perf_mode == 3;

        // desired idx update
        let mut any_step = false;
        if beyond {
            cpu0.force_idx(cpu0.freqs.len() - 1, now);
            cpu2.force_idx(cpu2.freqs.len() - 1, now);
            cpu5.force_idx(cpu5.freqs.len() - 1, now);
            cpu7.force_idx(cpu7.freqs.len() - 1, now);
        ...
        // apply caps
        let effective_zone = if beyond { TempZone::Cool } else { zone };
